...
tplfiles=`find $base ! -regex '.*/unittests/.*' -type f -regex '.*\.tpl' |wc -l`
...
find $base ! -regex '.*/unittests/.*' -type d | sed "s#$base#.#" | xargs -I '{}' mkdir -p $build_temp/{}
nrpanfiles=`find $base ! -regex '.*/unittests/.*' -type f -regex '.*\.pan' |wc -l`
panfiles=`find $base ! -regex '.*/unittests/.*' -type f -regex '.*\.pan'`
panc-annotations --base-dir $base --output-dir $build_temp $panfiles

nrannofiles=`find $build_temp -type f -regex '.*annotation\.xml'|wc -l`
if [ $nrpanfiles != $nrannofiles ]; then
    echo "[ERROR]: number of annotation files $nrannofiles is not the same as numnber of pan files $nrpanfiles."
    exit 1
else
...
# variable OS_VERSION_PARAMS already defined
...
find $base/pan -type f ! -name *.pan | xargs sed -n "s/^declaration[ ]\+template[ ]\+\(.*\);/include '\1';/p" | sort >> $build_temp/test.pan
find $base ! -regex '.*/unittests/.*' -type f ! -regex '^./pan/.*' -name *.pan | xargs sed -n "s/^declaration[ ]\+template[ ]\+\(.*\);/include '\1';/p" | sort >> $build_temp/test.pan
find $base ! -regex '.*/unittests/.*' -type f -name *.pan | xargs sed -n "s/^\(unique[ ]\+\)\?template[ ]\+\(.*\);/include '\2';/p" |sort >> $build_temp/test.pan
